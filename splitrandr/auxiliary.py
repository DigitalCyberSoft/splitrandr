...
class NamedSize:
    ...
    def __init__(self, size, name, refresh_rate=None):
        self._size = size
        self.name = name
...
class MonitorGeometry:
    """Parsed data from xrandr --listmonitors for a single monitor entry."""

    def __init__(self, name, width, height, x, y, w_mm, h_mm, output):
        self.name = name
        self.width = width
        self.height = height
        self.x = x
    ...
    def __repr__(self):
        return '<MonitorGeometry %s %dx%d/%dx%d+%d+%d %s>' % (
            self.name, self.width, self.height, self.w_mm, self.h_mm,
